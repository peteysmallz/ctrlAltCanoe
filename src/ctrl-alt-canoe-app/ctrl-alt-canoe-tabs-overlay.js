...
class CtrlAltCanoeTabsOverlay extends PolymerElement {
  ...
  static get is() { return 'ctrl-alt-canoe-tabs-overlay'; }
  ...
  _targetChanged(newTarget, oldTarget) {
    if (!this._transitionsInFlight.length) {
      if (this._lastTarget) {
        this._lastTarget.classList.remove('ctrl-alt-canoe-tabs-overlay-static-above');
      }
  ...
  _moveComplete() {
    if (this._lastTarget !== this.target) {
      ...
      if (this._lastTarget) {
        this._lastTarget.classList.add('ctrl-alt-canoe-tabs-overlay-static-above');
      }
...
customElements.define(CtrlAltCanoeTabsOverlay.is, CtrlAltCanoeTabsOverlay);
